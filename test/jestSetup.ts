...
if (j) {
    ...
    // (já mockado acima)
    // Evita conflito de act aninhado no @testing-library/react-native em alguns cenários
    try {
        const real = j.requireActual('@testing-library/react-native');
        j.mock('@testing-library/react-native', () => ({
            ...real,
            act: (callback: any) => {
                // executa callback diretamente para evitar erro 'root on unmounted test renderer'
                return callback();
            },
        }));
    } catch { /* módulo pode não estar disponível aqui */ }
}

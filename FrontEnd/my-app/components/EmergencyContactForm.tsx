  ...
  return (
    <View style={{ padding: 16, gap: 12 }}>
      <View>
        ...
        <TextField value={name} onChangeText={setName} placeholder="Nome" />
        {serverErrors?.name ? <Text style={{ color: '#FFFFFF', marginTop: 6 }}>{serverErrors.name}</Text> : null}
      </View>
      <View>
        <Text style={{ fontWeight: '700' }}>Telefone</Text>
        <TextField value={phone} onChangeText={setPhone} placeholder="+5511999999999" keyboardType="phone-pad" />
        {serverErrors?.phone ? <Text style={{ color: '#FFFFFF', marginTop: 6 }}>{serverErrors.phone}</Text> : null}
      </View>
      <View>
        <Text style={{ fontWeight: '700' }}>Mensagem Personalizada (opcional)</Text>
        <TextField
          value={customMessage}
          ...
          numberOfLines={3}
        />
        {serverErrors?.customMessage ? <Text style={{ color: '#FFFFFF', marginTop: 6 }}>{serverErrors.customMessage}</Text> : null}
      </View>
      <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }}>
        <Text style={{ fontWeight: '700' }}>Ativo</Text>
      ...
      {maxActiveReached && !(initial?.isActive) ? (
        <Text style={{ color: '#FFFFFF' }}>Você já possui 5 contatos ativos. Para ativar este, desative outro primeiro.</Text>
      ) : null}
      <TouchableOpacity

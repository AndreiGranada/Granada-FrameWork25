      ...
      >
        <Card elevated style={{ padding: Spacing.md, marginBottom: Spacing.md }}>
          <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
            ...
            <TouchableOpacity
              onPress={() => {
                trackEvent('reminder_list_edit_click', { id: item.id });
                router.push(`/reminders/${item.id}` as any);
              }}
              accessibilityRole="button"
              accessibilityLabel={`Editar lembrete ${item.name}`}
              style={{ paddingVertical: 6, paddingHorizontal: 10, borderRadius: 8, borderWidth: 1, borderColor: palette.border }}
            >

...
function main() {
    ...
    const IGNORED_FILES = new Set([
        'core/request.ts', // customizado no frontend para normalizar Authorization e cabeçalhos
    ]);
    const allKeys = new Set([...frontFiles.keys(), ...backFiles.keys()]);

    for (const key of allKeys) {
        const normalizedKey = key.replace(/\\/g, '/').replace(/^\/?sdk-backend\//, '').replace(/^\/?/, '');
        if (IGNORED_FILES.has(normalizedKey)) continue;

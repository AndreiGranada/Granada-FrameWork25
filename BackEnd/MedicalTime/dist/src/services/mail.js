...
Equipe MedicalTime
            ...
            if (process.env.EMAIL_DEV_LOG === 'true') {
                ...
                if (((_a = process.env.SMTP_HOST) === null || _a === void 0 ? void 0 : _a.includes('ethereal.email')) && info.preview) {
                    console.log('[MAIL:PREVIEW] Preview URL:', info.preview);
                }
            }
            return info;
        }
...
function sendEmail(params) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if (process.env.EMAIL_DEV_LOG === 'true') {
            ...
            if (text)
                console.log('[MAIL:DEV] Texto:', text.slice(0, 200));
            if (html)
                console.log('[MAIL:DEV] HTML length:', html.length);
        }
        // Se não há configurações SMTP, apenas loga (modo desenvolvimento)
        if (!process.env.SMTP_HOST || !process.env.SMTP_USER || !process.env.SMTP_PASS) {

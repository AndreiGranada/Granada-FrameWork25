...
const mailSchema = z.object({
    to: z.string().email(),
    subject: z.string().min(1).max(200).default('Teste de e-mail - MedicalTime'),
...
router.post('/test-mail', async (req: AuthRequest, res: Response): Promise<void> => {
    try {
        ...
        if (isDev && err?.message) return errorHelpers.internal(res, `Erro interno: ${err.message}`);
        errorHelpers.internal(res);
    }
});
